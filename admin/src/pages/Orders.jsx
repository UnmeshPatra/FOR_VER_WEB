...
const Orders = ({ token }) => {
  ...
  const statusHandler = async (event , ordersId) =>{
    try {
      const response = await axios.post(backendUrl + '/api/order/status' , {ordersId,status:event.target.value},{headers:{token}})
      if (response.data.success) {
      ...
      <div>
        {
          orders.map((order, index) => (
            <div className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr] gap-3 items-start border-2 border-gray-200 p-5 md:p-8 my-3 md:my-4 text-xs sm:text-sm text-gray-700' key={index}>
                ...
                <div>
                  {
                    ...
                    Array.isArray(order.items) && order.items.map((item, itemIndex) => {
                      if (itemIndex === order.items.length - 1) { // Correct condition for the last item
                        return (
              ...
              <p className='text-sm sm:test-[15px]'>{currency}{order.amount}</p>
